...
class Search extends SearchCore
{
    public static function find($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position', $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
        ...
        // If results, we retrieve stats
        if (isset($find['result']) && !empty($find['result']) &&
            Module::isInstalled('eatlivr')) {
            ...
            $grades_comments = eatlivrComment::getInfosOnProductsList($id_product_list);
            ...
            foreach ($products as $kp => $p) {
                foreach ($grades_comments as $gc) {
                    if ($gc['id_product'] == $p['id_product']) {
                        $products[$kp]['eatlivr']['grade_avg'] = round($gc['grade_avg']);
                        $products[$kp]['eatlivr']['nb_comments'] = $gc['nb_comments'];
                    }
                }
            }
